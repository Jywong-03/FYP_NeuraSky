...
def generate_flight_data():
    ...
    for year in years:
        ...
        for _ in range(NUM_FLIGHTS):
            ...
            crs_arr_time_obj = datetime(year, 1, 1, hour, minute) + timedelta(minutes=duration)
            crs_arr_time = crs_arr_time_obj.hour * 100 + crs_arr_time_obj.minute
            
            # Delays
            is_delayed = random.random() < 0.5 # 50% chance of delay (Balanced Dataset)
            dep_delay = 0
            if is_delayed:
                dep_delay = int(np.random.exponential(30)) # Exponential distribution for delay
                
            dep_time = (datetime(year, 1, 1, hour, minute) + timedelta(minutes=dep_delay)).strftime('%H%M')
        ...
        # Save
        output_path = f'dataset/Combined_Flights_{year}.csv'
